...
# --- Entity note assembly ----------------------------------------------------

assemble_entity_note <- function(entity_name, entity_type, facts,
                                 source_episode_ids = NULL) {
  if (!is.null(source_episode_ids) && length(source_episode_ids) > 0) {
    app_lines <- vapply(source_episode_ids, function(ep) paste0("- [[", ep, "]]"), character(1))
    appearances <- paste(app_lines, collapse = "\n")
  } else {
    appearances <- "-"
  }

  if (entity_type == "npc") {
    .assemble_npc_note(entity_name, facts, appearances)
  } else if (entity_type == "location") {
    .assemble_location_note(entity_name, facts, appearances)
  } else if (entity_type == "faction") {
    .assemble_faction_note(entity_name, facts, appearances)
  } else {
    stop("Unknown entity_type: ", entity_type)
  }
}

.assemble_npc_note <- function(entity_name, facts, appearances) {
  frontmatter <- glue(
    "---",
    "tags: [npc]",
    "name: {entity_name}",
  ...
  actions <- facts$actions %||% character(0)
  if (length(actions) > 0) {
    overview_lines <- vapply(actions, function(a) paste0("- ", a), character(1))
    overview_section <- paste(overview_lines, collapse = "\n")
  } else {
    overview_section <- "-"
  }

  quotes <- facts$quotes %||% character(0)
  if (length(quotes) > 0) {
    quote_blocks <- vapply(quotes, function(q) paste0("> ", q), character(1))
    quotes_section <- paste(quote_blocks, collapse = "\n\n")
  } else {
    quotes_section <- "-"
  }
...
.assemble_location_note <- function(entity_name, facts, appearances) {
  frontmatter <- glue(
    "---",
    "tags: [location]",
    "name: {entity_name}",
...
.assemble_faction_note <- function(entity_name, facts, appearances) {
  frontmatter <- glue(
    "---",
    "tags: [faction]",
    "name: {entity_name}",
