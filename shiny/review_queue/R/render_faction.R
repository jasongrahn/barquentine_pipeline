render_faction_review <- function(row) {
  ...
  vault_rel  <- file.path("factions", paste0(entity_id, ".md"))
  ...
  faction_fields <- list(
    ...
    list(key = "goals",       label = "Goals"),
    list(key = "members",     label = "Members"),
    list(key = "structure",   label = "Structure / hierarchy"),
  ...
  tagList(
    fluidRow(
      column(8,
        tags$h4(style = "margin-bottom:2px;", entity_name,
          tags$span(style = "font-size:0.65em;color:#666;font-weight:400;margin-left:8px;", "FACTION")
        ),
    ...
    if (scan_for_placeholders(draft_text)) render_placeholder_banner(),
    ...
    fluidRow(
      ...
      column(6,
        ...
        tabsetPanel(
          id = "draft_tabs",
          tabPanel("Card",
            .render_structured_draft(fm, body, faction_fields)
          ),
